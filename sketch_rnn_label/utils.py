...
class DataLoader(object):
    ...
    def __init__(self,
                 ...
                 limit=1000):
        # alert here the self.strokes include strokes and labels, therefore format is (stroke, label)
        self.strokes = []
    ...
    def preprocess(self, strokes, labels):
    ...
    def random_scale(self, data):
        ...
        result = np.copy(data)
        result[0][:, 0] *= x_scale_factor
        result[0][:, 1] *= y_scale_factor
        return result
    ...
    def calculate_normalizing_scale_factor(self):
        ...
        for i in range(len(self.strokes)):
            if len(self.strokes[i][0]) > self.max_seq_length:
                continue
            for j in range(len(self.strokes[i][0])):
                data.append(self.strokes[i][0][j, 0])
                data.append(self.strokes[i][0][j, 1])
        data = np.array(data)
        return np.std(data)
    ...
    def normalize(self, scale_factor=None):
        ...
        for i in range(len(self.strokes)):
            self.strokes[i][0][:, 0:2] /= self.scale_factor
    ...
    def _get_batch_from_indices(self, indices):
        ...
        for idx in range(len(indices)):
            ...
            if self.augment_stroke_prob > 0:
                data_copy[0] = augment_strokes(data_copy[0], self.augment_stroke_prob)
            x_batch.append(data_copy[0])
            lalbel_batch.append(data_copy[1])
            length = len(data_copy[0])
            seq_len.append(length)
        seq_len = np.array(seq_len, dtype=int)
        # We return three things: stroke-3 format, stroke-5 format, list of seq_len.
        return x_batch, lalbel_batch, self.pad_batch(x_batch, self.max_seq_length), seq_len
