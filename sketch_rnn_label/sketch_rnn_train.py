...
def load_dataset(data_dir, model_params, inference_mode=False):
    ...
    for index, dataset in enumerate(datasets):
        ...
        if train_strokes is None:
            ...
            train_class = np.array([index]*len(data['train']))
            valid_strokes = data['valid']
            valid_class = np.array([index]*len(data['valid']))
            test_strokes = data['test']
            test_class = np.array([index]*len(data['test']))
        else:
            train_strokes = np.concatenate((train_strokes, data['train']))
            valid_strokes = np.concatenate((valid_strokes, data['valid']))
            test_strokes = np.concatenate((test_strokes, data['test']))
            train_class = np.concatenate((train_class, np.array([index]*len(data['train']))))
            valid_class = np.concatenate((valid_class, np.array([index]*len(data['valid']))))
            test_class = np.concatenate((test_class, np.array([index]*len(data['test']))))
...
def evaluate_model(sess, model, data_set):
    ...
    for batch in range(data_set.num_batches):
        unused_orig_x, label, x, s = data_set.get_batch(batch)
        feed = {model.input_data: x, model.input_label_data: label, model.sequence_lengths: s}
        (cost, r_cost,
...
def train(sess, model, eval_model, train_set, valid_set, test_set):
    ...
    for _ in range(hps.num_steps):
        ...
        _, label, x, s = train_set.random_batch()
        feed = {
            model.input_data: x,
