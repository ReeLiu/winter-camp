...
class Model(object):
    ...
    def encoder(self, batch, label, sequence_lengths, conditioned=True):
        ...
        label_embedding = tf.nn.embedding_lookup(self.input_label_embedding_matrix, label)
        last_h = tf.concat([last_h_fw, last_h_bw, label_embedding], 1)
        mu = rnn.super_linear(
            last_h,
            self.hps.z_size,
            input_size=self.hps.enc_rnn_size * 2 + self.hps.label_embedding_dim,  # bi-dir, so x2
            scope='ENC_RNN_mu',
            init_w='gaussian',
            weight_start=0.001)
    ...
    def build_model(self, hps):
        ...
        self.input_label_data = tf.placeholder(dtype=tf.int32, shape=[self.hps.batch_size])
        self.input_label_embedding_matrix = tf.get_variable('label_embedding_matrix',
                                                            [len(self.hps.data_set), self.hps.label_embedding_dim],
        ...
        if hps.conditional:  # vae mode:
            self.mean, self.presig = self.encoder(self.output_x, self.input_label_data,
                                                  self.sequence_lengths, conditioned=True)
            ...
            # add here to include label information
            self.mean, self.presig = self.encoder(self.output_x, self.input_label_data,
                                                  self.sequence_lengths, conditioned=False)
            # alert here not introduce gaussian noise
